...
export default function Golden(){
 ...
 return <main className={`golden ${step===0?'golden-intro':'golden-mission'}`}>
  ...
  {step>0&&<nav className="g-steps" aria-label="미션 진행">{places.map((p,i)=><button type="button" onClick={()=>alert(scanMessage)} aria-current={step===i+1?'step':undefined} key={p}><span>{[s.lanterns===3,s.goods===3,s.seal][i]?<Check size={18}/>:i+1}</span>{p}{ready&&!allowed(i+1,s)&&<LockKeyhole size={14}/>}</button>)}</nav>}
