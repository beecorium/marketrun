...
export default function Golden(){
 ...
 return <main className={`golden ${step===0?'golden-intro':'golden-mission'}`}>
  ...
  {step===1?<><p>망루의 세 불빛이 사라졌습니다. 휴대폰 플래시를 켜고 현장 등불의 반사 글자를 찾아, 등불마다 안내된 글자 수에 맞춰 순서대로 입력하세요.</p><div className="g-lanterns">{[0,1,2].map(i=><div className={s.lanterns>i?'lit':''} key={i}><Lamp size={62} strokeWidth={1.2}/><b>{s.lanterns>i?lanternAnswers[i]:`${i+1}번째`}</b></div>)}</div><p className="g-small">등불 점등 {s.lanterns}/3 · {clueHints[s.lanterns]??'세 등불을 모두 밝혔습니다.'}</p></>:step===2?<><p>현장 카드에서 보령 3미를 찾아, 아래 번호를 확인한 후 나온 순서대로 세 자리 암호를 입력하세요.</p><div className="g-tastes">{[0,1,2].map(i=><span className={s.goods>i?'found':''} key={i}><b>{s.goods>i?<><span className="g-taste-icon" aria-hidden="true">{['🍇','🐙','🦪'][i]}</span><span className="g-taste-name">{['포도','주꾸미','굴'][i]}</span></>:`${i+1}번째 맛`}</b>{s.goods<=i&&<small>단서를 찾아주세요</small>}</span>)}</div></>:<><div className="g-statue"><img src={awake||done?'/images/peddler-awake-golden.png':'/images/peddler-sleeping-golden.png'} alt={awake||done?'바구니를 열어 포도·굴·주꾸미를 담는 깨어난 보부상':'닫힌 바구니를 끌어안고 잠든 보부상'}/>{!awake&&!done&&<span>보부상이 닫힌 바구니를 안고 잠들어 있습니다</span>}</div><p>신입 야행 보부상이여…<br/>포목거리에서 발견한 황금빛 물건의 이름을 말해보게.</p>{!done&&<><button className="secondary" disabled={hint===3} onClick={()=>H(Math.min(3,hint+1))}>힌트 보기 {hint}/3</button>{hint>0&&<p className="g-small">{['황금빛으로 빛나고 있었습니다.','가지와 잎이 있습니다.','보령시의 상징물인 소나무의 모습을 하고 있습니다.'][hint-1]}</p>}</>}</>}
