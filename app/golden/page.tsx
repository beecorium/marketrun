...
export default function Golden(){
 ...
 useEffect(()=>{document.title='황금 패랭이를 찾아라 | 보령 꿀잼야행';const params=new URLSearchParams(location.search);const mission=params.get('mission');const n=mission===null?0:Number(mission);T([0,1,2,3,4].includes(n)?n:0);try{S(normalize(JSON.parse(localStorage.getItem(key)||'null')));}catch{SE(true);}R(true);const sync=()=>{try{S(normalize(JSON.parse(localStorage.getItem(key)||'null')));}catch{SE(true);}};window.addEventListener('storage',sync);fetch('/api/config',{cache:'no-store'}).then(r=>{if(!r.ok)throw Error();return r.json();}).then(data=>{
  ...
  }).catch(()=>E(true));return()=>{window.removeEventListener('storage',sync);};},[]);
 ...
 return <main className={`golden ${step===0?'golden-intro':'golden-mission'}`}>
  {step===0?<img className="g-main-banner" src="/images/golden-main-banner.png" alt="황금 패랭이를 찾아라 – 보령 꿀잼야행"/>:<><header><img src="/images/market-run-logo.png" alt="마켓런"/><span>보령 꿀잼야행</span><button className="icon-btn" aria-label="여정 안내" onClick={()=>D(true)}><MapPin/></button></header>
  ...
  {done&&<div className="g-success"><strong className="g-stamp">CLEAR</strong><div className="g-earned"><span className="g-earned-image"><img src={prizeImages[step-1]} alt={`${prizes[step-1]} 획득 아이콘`}/></span><small>MISSION {String(step).padStart(2,'0')} 징표 획득</small><h3>{step===1?'「황금갓끈」 획득!':step===2?<>「황금 동백꽃 장식」<br/>획득!</>:'「황금 인장」 획득!'}</h3>{step===2&&<p className="g-small">보령시 시화 동백꽃을 황금 패랭이에 장식했습니다.</p>}</div>{step===2&&<p>포목거리를 지나며 황금빛 그림을 찾아 기억하세요. 마지막 QR은 보부상 <br/>조형물 옆에 있습니다.</p>}{step===3&&<p>황금 패랭이가 마침내 완전한 모습을 드러냈습니다.</p>}{step<3?<button onClick={()=>D(true)}>{step===1?'한내시장 쉼터로 이동하기':'포목거리 탐색 시작하기'}</button>:<a className="g-button" href={link(4)}>황금패랭이 쓰고 인증샷 찍기</a>}</div>}</section>}
