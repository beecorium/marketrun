...
import {allowed,advance,fresh,key,normalize,lanternAnswers,qrMissions,type Journey} from './progress';
import './style.css';
...
const scanMessage='미션 장소에서 QR을 스캔해주세요';
const missionUrls=['/golden?checkpoint=lantern-Q7m4K9v2R6','/golden?checkpoint=three-tastes-N8c5W2p7H4','/golden?checkpoint=golden-seal-T3x9B6k2M7'];
export default function Golden(){
 ...
 const link=(n:number)=>n===4?'/golden/?mission=4':'/golden/';
 ...
 return <main className={`golden ${step===0?'golden-intro':'golden-mission'}`}>
  {step===0?<img className="g-main-banner" src="/images/golden-main-banner.png" alt="황금 패랭이를 찾아라 – 보령 꿀잼야행"/>:<><header><img src="/images/market-run-logo.png" alt="마켓런"/><span>황금패랭이를 찾아라</span><button className="icon-btn" aria-label="여정 안내" onClick={()=>D(true)}><MapPin/></button></header>
  <div className="g-heading"><span className="g-eyebrow">보령 밤길에 숨은 세 개의 징표</span></div></>}
  {step>0&&<nav className="g-steps" aria-label="미션 진행">{places.map((p,i)=>{const complete=[s.lanterns===3,s.goods===3,s.seal][i];const content=<><span>{complete?<Check size={18}/>:i+1}</span>{p}{ready&&!allowed(i+1,s)&&<LockKeyhole size={14}/>}</>;return complete?<a href={missionUrls[i]} aria-current={step===i+1?'step':undefined} key={p}>{content}</a>:<button type="button" onClick={()=>alert(scanMessage)} aria-current={step===i+1?'step':undefined} key={p}>{content}</button>;})}</nav>}
  {!ready?<p role="status">여정을 불러오고 있습니다…</p>:step===0?<section className="g-card g-intro"><h2>지금부터 여러분은<br/>신입 야행 보부상입니다!</h2><p>망루에서 첫 번째 징표를 찾고,<br/>한내시장을 지나 두 번째 징표를<br/>찾으세요.<br/>그리고 마지막 보부상을 만나<br/>황금 패랭이를 완성하세요!</p><a className="g-button" href="/golden?checkpoint=lantern-Q7m4K9v2R6">도전하기</a></section>:storageError?<section className="g-card"><h2>진행 기록 저장을 확인해주세요</h2><p>같은 휴대폰의 일반 브라우저에서 모든 QR을 열어주세요. 비공개 모드나 저장 차단 설정에서는 진행 기록을 이어갈 수 없습니다.</p><button onClick={()=>location.reload()}>다시 확인</button></section>:!allowed(step,s)?<section className="g-card g-locked"><LockKeyhole size={42}/><h2>아직 봉인된 미션입니다</h2><p>{s.lanterns<3?'망루에서 첫 번째 미션을 먼저 완료해주세요.':s.goods<3?'한내시장 쉼터에서 두 번째 미션을 먼저 완료해주세요.':'보부상 조형물에서 황금 인장을 찍어주세요.'}</p><button className="g-button" onClick={()=>alert(scanMessage)}>QR 스캔 안내</button><p className="g-small">진행 기록은 같은 휴대폰·같은 브라우저에서 이어집니다.</p></section>:step===4?rewardReady?<section className="g-card g-reward-proof"><ShieldCheck size={62}/><span className="g-eyebrow">만족도 조사 참여 확인</span><h2>경품 교환<br/>완료 화면</h2><div className="g-proof-seal"><span>MISSION</span><strong>COMPLETE</strong><small>경품 증정 가능</small></div><p>이 화면을 현장 운영요원에게 보여주세요.<br/><strong>운영요원 확인 후 경품이 증정됩니다.</strong></p><time>{rewardTime}</time><p className="g-proof-warning">※ 경품 수령 전에는 이 화면을 닫지 마세요.</p><button className="g-reset" onClick={()=>{if(confirm('미션 진행 기록을 초기화하고 처음부터 다시 시작할까요?')&&save(fresh()))location.href=link(0);}}>처음부터 다시하기</button></section>:<section className="g-card g-finish"><span className="g-eyebrow">세 개의 징표, 하나의 빛</span><img className="g-hat" src={hatImage} alt="갓끈과 보령시 시화 동백꽃, 황금 인장으로 완성한 황금 패랭이"/><h2>수석 야행 보부상으로<br/>임명합니다!</h2><p>보령의 시장길을 따라 모든 징표를 찾았습니다.<br/>보령시 시화 동백꽃으로 장식된 황금 패랭이와 함께 오늘의 빛나는 순간을 남겨보세요.</p><button onClick={()=>P(true)}><Camera size={20}/>황금패랭이 인증사진찍기</button><section className="g-survey-gate"><h3>경품 수령 전 필수</h3><p>만족도 조사를 제출한 뒤 이 페이지로 돌아와 완료 버튼을 눌러주세요.</p>{surveyLink}{config?.surveyUrl&&<><button className="secondary" disabled={!surveyOpened} onClick={confirmSurvey}><Check size={19}/>설문 제출을 완료했어요</button>{!surveyOpened&&<p className="g-small">설문 링크를 먼저 열어야 완료 버튼이 활성화됩니다.</p>}</>}<p className="g-survey-notice">완료 후 표시되는 경품 교환 화면을 현장에서 보여주셔야 경품을 받을 수 있습니다.</p></section></section>:<section className={'g-card '+(done?'g-clear':'')}><h2>{titles[step-1]}</h2>
  {step===1?<><p>망루의 세 불빛이 사라졌습니다. 휴대폰 플래시를 켜고 현장 등불의 반사 글자를 찾아, 등불마다 안내된 글자 수에 맞춰 순서대로 입력하세요.</p><div className="g-lanterns">{[0,1,2].map(i=><div className={s.lanterns>i?'lit':''} key={i}><Lamp size={62} strokeWidth={1.2}/><b>{s.lanterns>i?lanternAnswers[i]:`${i+1}번째`}</b></div>)}</div><p className="g-small">등불 점등 {s.lanterns}/3 · {clueHints[s.lanterns]??'세 등불을 모두 밝혔습니다.'}</p></>:step===2?<><p>현장 카드에서 보령 3미를 찾아, 번호를 확인한 후 아래 순서대로 세 자리 암호를 입력하세요.</p><div className="g-tastes">{[0,1,2].map(i=><span className={s.goods>i?'found':''} key={i}><b>{s.goods>i?<><span className="g-taste-icon" aria-hidden="true">{['🦪','🍇','🐙'][i]}</span><span className="g-taste-name">{['굴','포도','주꾸미'][i]}</span></>:`${i+1}번째 맛`}</b>{s.goods<=i&&<small>{['힌트:바다의 우유','힌트:보라빛 알맹이','힌트:다리가 8개!!'][i]}</small>}</span>)}</div></>:<><div className="g-statue"><img src={awake||done?'/images/peddler-awake.gif':'/images/peddler-sleeping-golden.png'} alt={awake||done?'바구니를 열어 포도·굴·주꾸미를 담는 깨어난 보부상':'닫힌 바구니를 끌어안고 잠든 보부상'}/>{!awake&&!done&&<span>보부상이 닫힌 바구니를 안고 잠들어 있습니다</span>}</div><p>신입 야행 보부상이여…<br/>포목거리에서 발견한 황금빛 물건의 이름을 말해보게.</p>{!done&&<><button className="secondary" disabled={hint===3} onClick={()=>H(Math.min(3,hint+1))}>힌트 보기 {hint}/3</button>{hint>0&&<p className="g-small">{['황금빛으로 빛나고 있었습니다.','가지와 잎이 있습니다.','보령시를 대표하는 나무(시목)의 모습을 하고 있습니다.'][hint-1]}</p>}</>}</>}
  {!done&&!awake&&<form onSubmit={submit}><label className={step===2?'sr-only':undefined} htmlFor="g-answer">{step===1?`${s.lanterns+1}번째 등불의 글자`:step===2?'세 자리 암호':'포목거리의 황금빛 물건'}</label><div className="g-input"><input id="g-answer" value={input} onChange={e=>I(e.target.value)} autoComplete="off" inputMode={step===2?'numeric':'text'} aria-describedby={step===1?'g-answer-hint':undefined} maxLength={step===3?20:step===2?3:s.lanterns===2?2:1} placeholder={step===3?'이름을 입력하세요':step===2?'세 자리 암호':s.lanterns===2?'두 글자를 입력하세요':'한 글자를 입력하세요'} required/><button type="submit">{step===1?'등불 밝히기':step===2?'암호 확인':'봉인 풀기'}</button></div>{step===1&&<p id="g-answer-hint" className="g-small">{s.lanterns===2?'두 글자 입력':'한 글자 입력'}</p>}</form>}
  ...
  {done&&<div className="g-success"><div className="g-earned"><span className="g-earned-image"><img src={prizeImages[step-1]} alt={`${prizes[step-1]} 획득 아이콘`}/></span><h3>{step===1?'「황금갓끈」 획득!':step===2?<>「황금 동백꽃 장식」<br/>획득!</>:'「황금 인장」 획득!'}</h3>{step===2&&<p className="g-small">보령을 대표하는 시화 동백꽃을 황금 패랭이에 장식했습니다.</p>}</div>{step===2&&<p>포목거리를 지나며 황금빛 그림을 찾아 기억하세요. 마지막 QR은 보부상 <br/>조형물 옆에 있습니다.</p>}{step<3?<button onClick={()=>D(true)}>{step===1?'한내시장 쉼터로 이동하기':'포목거리 탐색 시작하기'}</button>:<a className="g-button" href={link(4)}>황금패랭이 완성하고<br/>임명장 받기</a>}</div>}</section>}
  ...
  {ready&&new URLSearchParams(typeof location==='undefined'?'':location.search).get('test')==='1'&&<button className="g-reset" onClick={()=>{if(confirm('신규 황금 패랭이 진행 기록만 초기화할까요?')){if(save(fresh()))location.href=link(0);}}}>테스트 기록 초기화</button>}
 </main>;
}
